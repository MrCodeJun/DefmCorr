...
def pairwise_dist(X,Y):
    """
        feature_matrix1：Batchsize*N*f_dim
        feature_matrix2：Batchsize*N*f_dim
        out_gt = Batch_size*N*N
        zj
    """
    X_ = tf.expand_dims(X, 1)
    Y_ = tf.expand_dims(Y, 2)
    s = X_ - Y_
    distance = tf.norm(s, axis=-1)
...
#     return tf.contrib.layers.batch_norm(data, decay = 0.9, center = True,scale = True,
...
def separable_conv2d(input, output, name, is_training, kernel_size, depth_multiplier=1,
                     reuse=None, with_bn=True, activation_fn=tf.nn.elu):
    conv2d = tf.layers.separable_conv2d(input, output, kernel_size=kernel_size, strides=(1, 1), padding='VALID',
...
#            reuse=None, with_bn=True, activation_fn=tf.nn.relu):
...
### tf.nn.elu
# def dense(input, output, name, is_training, reuse=None, with_bn=True, activation_fn=tf.nn.relu):
#     dense = tf.layers.dense(input, units=output, activation=None,
#                             kernel_initializer=tf.contrib.layers.xavier_initializer(),
#                             kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=1.0),
#                             reuse=reuse, name=name, use_bias=not with_bn)
#     if with_bn:
#         dense =  batch_normalization(dense, is_training, name + '_bn', reuse)
#     if activation_fn is not None:
#         dense = activation_fn(dense)
#
#     return dense

def dense(input, output, name, is_training, reuse=None, with_bn=True, activation_fn=tf.nn.elu):
    dense = tf.layers.dense(input, units=output, activation=activation_fn,
                            kernel_initializer=tf.glorot_normal_initializer(),
                            kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=1.0),
                            reuse=reuse, name=name, use_bias=not with_bn)
...
def max_pool3d(inputs, kernel_size, name ,stride=[2, 2, 2], padding='VALID'):
    """ 2D max pooling.
    ...
    Variable tensor
    """
    outputs = tf.layers.max_pooling3d(inputs,
                                      pool_size=kernel_size,
                                      strides=stride,
                                      padding=padding,
                                      name = name)
...
def average_pool2d(inputs, kernel_size, name ,stride=[2, 2], padding='VALID'):
  """ 2D max pooling.
