...
TEST_SHAPE_DIR = '/home/junz/Github/Data/CorrsTmplt'
TEST_SHAPE_LISTS = ['Airplane', 'Chair', 'Motorbike']
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            with tf.control_dependencies(extra_update_ops):
                train_op = optimizer.minimize(total_loss + reg_loss, global_step=batch)
        ...
        saver.restore(sess,os.path.join(LOG_DIR, "model35.ckpt"))
        eval_one_epoch(sess, ops, test_writer)


def eval_one_epoch(sess, ops, test_writer):
    for name in TEST_SHAPE_LISTS:
        temp_shape_list = sp.get_file_list(DATA_DIR, '.h5')
        shape_temp = os.path.join(DATA_DIR, temp_shape_list[1])
        f = h5py.File(shape_temp)
        templates = f['shape_temp'][:]
        points = f['points'][:]
        f.close()
        test_dir = os.path.join(TEST_SHAPE_DIR, name)
        points_files = sp.get_file_list(test_dir, '.h5')
        for file in points_files:
            file = test_dir +'/'+file
            points, _, _, samples = sp.generate_validation_query_points_h5(file,
                                                   is_fpfh=False,
                                                   shape_num=10000,
                                                   query_num=POINT_NUM,
                                                   input_num= POINT_NUM,
            ...
            Data = np.expand_dims(points[0:POINT_NUM, :], axis=0)
            Temp = np.expand_dims(templates[:, :], axis=0)
            ...
            if IS_SHOW =='True':
                ...
                mlab.points3d(10 * rotated_data[0, :, 0], 10 * rotated_data[0, :, 1], 10 * rotated_data[0, :, 2],
                              color=(0.7, 0.7, 0.7), scale_factor=0.2, scale_mode='vector')
                mlab.show()
            ...
            feed_dict = {ops['input_shapes']: rotated_data,
                         ops['template_shapes']: Temp,
                         ops['is_training_pl']: 'False'
                         }

            morhp_shapes = sess.run(ops['morph_shapes'], feed_dict=feed_dict)
