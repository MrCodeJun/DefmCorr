...
class DeformNet():
    def __init__(self, is_training, batch_size, point_num, dense_weight, is_trans, is_key):
        ...
        # input shape and deformation networks:
        ...
        if is_trans is not 'no_trans':
            ...
            self.shape_loss = self.shape_loss + 0.01*self.move_loss + 0.001 * self.lap_loss
        ...
        self.real_seg_preds = self.seg_shape(self.points_feats, 50, 'netSeg', self.is_training)
        self.real_seg_loss = self.real_local_seg_loss(self.real_seg_preds, self.point_labels)

        ## seg_net:
        if is_trans is not 'no_trans':
            self.points_feats2, self.global_fts2, self.transform_matrix2, self.points_t2 = self.point_feat_extract(
                                                                                                self.points_deform_transpose,
                                                                                                'netE',
                                                                                                False,
                                                                                                self.is_trans,
                                                                                                is_reused=True)
            self.transform_matrix_transpose2 = tf.transpose(self.transform_matrix2, perm=(0, 2, 1), name='transpose')
        else:
            self.points_feats2, self.global_fts2, self.points_t2 = self.point_feat_extract(self.points_deform, 'netE',
                                                                                       self.is_training, self.is_trans,
                                                                                           is_reused=True)
        # self.fake_seg_preds = self.seg_shape(self.points_feats2, 50, 'netSeg', is_training = False, is_resued=True)
        #
        # self.fake_seg_loss = self.fake_local_seg_loss(self.fake_seg_preds,
        #                                               self.points_deform_transpose,
        #                                               self.points, self.point_labels)
    ...
    def seg_shape(self, feats, parts_num, name, is_training, is_resued=None):
        net = pf.dense(feats, 1024, name+'mlp1', is_training, reuse=is_resued, activation_fn=tf.nn.relu)
        net = pf.dense(net, 512, name+'mlp2', is_training, reuse=is_resued, activation_fn=tf.nn.relu)
        net = pf.dense(net, 256, name+'mlp3', is_training, reuse=is_resued, activation_fn=tf.nn.relu)
        net = pf.dense(net, 128, name+'mlp4', is_training, reuse=is_resued, activation_fn=tf.nn.relu)
        net = pf.dense(net, parts_num, name+'mlp5', is_training, reuse=is_resued, activation_fn=None)
    ...
    def local_map_loss(self, temp_points, deform_points, K=8, D=1):
        if D==1:
            _, indices = pf.knn_indices_general(temp_points, temp_points, K, False)
        else:
            _, indices_dilated = pf.knn_indices_general(temp_points, temp_points, K * D, True)
            indices = indices_dilated[:, :, ::D, :]
        ...
        directions = offsets/tf.tile(tf.expand_dims(dists, axis=-1), [1, 1, 3])
        nn_directions = tf.gather_nd(directions, indices, name='nn_pts')    # batch*point*k*3
    ...
    def real_local_seg_loss(self,pred_seg_labels, seg_labels):

        # input the real shape and train the seg_net
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred_seg_labels, labels=seg_labels),
                              axis=1)
        classify_loss = tf.reduce_mean(loss)
        return classify_loss
